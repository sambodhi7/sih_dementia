...
export function VoiceGameLauncher({ languageCode, copy, onAudio }: { languageCode: string; copy: VoiceCopy; onAudio: (uri: string) => Promise<string | void> }) {
  ...
  return <View pointerEvents="box-none" style={styles.wrap}>
    {open ? <View accessibilityLiveRegion="polite" style={styles.tray}>
      <Text style={styles.trayTitle}>{recording.isRecording ? copy.listening : message || copy.example}</Text>
      {!recording.isRecording && !busy ? <Text style={styles.example}>{copy.example}</Text> : null}
      {recording.isRecording ? <Pressable accessibilityRole="button" accessibilityLabel={copy.stop} onPress={() => { touchFeedback(); void stop(); }} style={styles.stop}><Text style={styles.stopText}>■ {copy.stop}</Text></Pressable> : null}
      {!recording.isRecording && !busy ? <Pressable accessibilityRole="button" accessibilityLabel={copy.close} onPress={() => setOpen(false)}><Text style={styles.close}>{copy.close}</Text></Pressable> : null}
    </View> : null}
