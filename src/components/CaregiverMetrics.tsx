...
const games: Array<{ id: GameId; title: string; description: string }> = [
  ...
  { id: 'recipe', title: 'Recipe', description: 'Familiar steps and gentle sequence practice.' },
];
const percent = (value: number | null) => value === null ? 'Not enough data' : `${Math.round(value)}%`;
const seconds = (value: number | null) => value === null ? 'Not enough data' : `${value.toFixed(1)} sec`;

function trendCopy(direction: PatientProfileMetrics['trend']['direction']) {
  if (direction === 'improving') return 'Recent sessions needed less support than earlier sessions.';
  if (direction === 'changing') return 'Support needs are changing across recent sessions.';
  if (direction === 'stable') return 'Support needs are similar across recent sessions.';
...
export function CaregiverMetrics({ patientId, locale }: { patientId: string; locale: string }) {
  ...
  return <View style={styles.stack}>
    ...
    {snapshots.map((snapshot) => <GameCard key={snapshot.gameId} snapshot={snapshot} />)}
