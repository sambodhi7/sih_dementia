import { createInitialState, extract, onSessionEnd, REGISTRY } from '../services/adaptive';
...
export async function startDaysPlanSession(patientId: string, phase: 'morning' | 'evening', companionPresent = false): Promise<StoredDaysPlanSession> {
  const session: StoredDaysPlanSession = { id: makeId(), patientId, gameId: 'days_plan', phase, startedAt: Date.now(), endedAt: null, abandoned: false, companionPresent };
  if (Platform.OS === 'web') await queuedWrite(async () => { const snapshot = await loadWeb(); snapshot.sessions.push(session); await saveWeb(); });
  else { const db = await getDb(); await db.withTransactionAsync(async () => { await db.runAsync('INSERT INTO sessions (id, patient_id, game_id, started_at, phase, companion_present) VALUES (?, ?, ?, ?, ?, ?)', [session.id, patientId, session.gameId, session.startedAt, phase, companionPresent ? 1 : 0]); await enqueue(db, 'sessions', session.id); }); }
  return session;
}
...
export async function finishWhosWhoSession(session: StoredSession, fallbackOutcome: SessionOutcome = { scoredActions: 0, successRate: null, unassistedRate: null, medianLatencySeconds: null, wasAbandoned: false, unassistedLatencies: [], successfulScoredActions: 0, unassistedScoredActions: 0 }) {
  const now = Date.now();
  if (Platform.OS === 'web') {
...
export async function finishDaysPlanSession(session: StoredDaysPlanSession): Promise<void> {
  const now = Date.now();
  if (Platform.OS === 'web') {
    await queuedWrite(async () => {
      const snapshot = await loadWeb();
      const events = snapshot.events.filter((entry) => entry.sessionId === session.id).sort((a, b) => a.seq - b.seq).map((entry) => entry.event);
      const prior = snapshot.controllerStates.find((state) => state.patientId === session.patientId && state.gameId === session.gameId);
      const initial = prior ?? createInitialState(session.patientId, REGISTRY.days_plan, session.startedAt);
      const result = onSessionEnd(events, initial, REGISTRY.days_plan, now, session.phase, session.companionPresent);
      snapshot.sessions = snapshot.sessions.map((entry) => entry.id === session.id ? { ...entry, endedAt: now } : entry);
      snapshot.outcomes.push({ sessionId: session.id, outcome: extract(events, session.phase, session.companionPresent) });
      snapshot.controllerStates = [...snapshot.controllerStates.filter((state) => !(state.patientId === session.patientId && state.gameId === session.gameId)), result.state];
      snapshot.controllerStateChanges.push({ id: makeId(), state: result.state, sessionId: session.id, source: result.next.source, at: now });
      await saveWeb();
    });
  ...
  await queuedWrite(async () => { await db.withTransactionAsync(async () => {
    await db.runAsync('UPDATE sessions SET ended_at = ? WHERE id = ?', [now, session.id]);
    const rows = await db.getAllAsync<{ payload: string }>('SELECT payload FROM events WHERE session_id = ? ORDER BY seq', [session.id]);
    const events = rows.map((row) => JSON.parse(row.payload) as GameEvent);
    const outcome = extract(events, session.phase, session.companionPresent);
    await db.runAsync('INSERT OR REPLACE INTO session_outcomes (session_id, scored_actions, success_rate, unassisted_rate, median_latency_seconds, was_abandoned, successful_scored_actions, unassisted_scored_actions, unassisted_latencies) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [session.id, outcome.scoredActions, outcome.successRate, outcome.unassistedRate, outcome.medianLatencySeconds, outcome.wasAbandoned ? 1 : 0, outcome.successfulScoredActions, outcome.unassistedScoredActions, JSON.stringify(outcome.unassistedLatencies)]);
    const prior = await db.getFirstAsync<{ difficulty: number; hint_time_seconds: number; sessions_observed: number; latency_samples: string }>('SELECT difficulty, hint_time_seconds, sessions_observed, latency_samples FROM controller_state WHERE patient_id = ? AND game_id = ?', [session.patientId, session.gameId]);
    const state = prior ? { patientId: session.patientId, gameId: session.gameId, difficulty: prior.difficulty, hintTimeSeconds: prior.hint_time_seconds, sessionsObserved: prior.sessions_observed, latencySamples: JSON.parse(prior.latency_samples) as number[], updatedAt: now } : createInitialState(session.patientId, REGISTRY.days_plan, session.startedAt);
    const result = onSessionEnd(events, state, REGISTRY.days_plan, now, session.phase, session.companionPresent);
    await db.runAsync('INSERT OR REPLACE INTO controller_state (patient_id, game_id, difficulty, hint_time_seconds, sessions_observed, latency_samples, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [session.patientId, session.gameId, result.state.difficulty, result.state.hintTimeSeconds, result.state.sessionsObserved, JSON.stringify(result.state.latencySamples), result.state.updatedAt]);
    const trajectoryId = makeId();
    await db.runAsync('INSERT INTO controller_state_changed (id, patient_id, game_id, session_id, difficulty, hint_time_seconds, source, at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [trajectoryId, session.patientId, session.gameId, session.id, result.state.difficulty, result.state.hintTimeSeconds, result.next.source, now]);
    await enqueue(db, 'session_outcomes', session.id, now);
...
export async function abandonDaysPlanSession(session: StoredDaysPlanSession): Promise<void> {
...
export async function readPatientSessionRecords(patientId: string, gameId?: GameId): Promise<SessionRecord[]> {
  ...
  if (Platform.OS === 'web') { const snapshot = await loadWeb(); return snapshot.sessions.filter((session) => session.gameId !== 'recipe' && session.patientId === patientId && session.endedAt !== null && (!gameId || session.gameId === gameId)).map((session) => ({ session: { gameId: session.gameId, startedAt: session.startedAt, companionPresent: session.companionPresent, ...('phase' in session ? { phase: session.phase } : {}) }, events: snapshot.events.filter((event) => event.sessionId === session.id).sort((a, b) => a.seq - b.seq).map((event) => event.event) })); }
  const db = await getDb();
  const sessions = await db.getAllAsync<{ id: string; game_id: GameId; started_at: number; phase: 'morning' | 'evening' | null; companion_present: number }>(`SELECT id, game_id, started_at, phase, companion_present FROM sessions WHERE patient_id = ? AND ended_at IS NOT NULL ${gameId ? 'AND game_id = ?' : "AND game_id IN ('days_plan', 'whos_who')"} ORDER BY started_at ASC`, gameId ? [patientId, gameId] : [patientId]);
