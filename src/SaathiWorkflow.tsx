...
import { DaysPlanActivity, DaysPlanEditor, type DaysPlanItem } from './components/daysPlan';
...
import type { StoredDaysPlanSession, StoredSession, WhosWhoDraft, WhosWhoItem } from './storage/types';
import { theme } from './theme';
import { touchFeedback } from './lib/haptics';


type Screen = 'language' | 'onboarding' | 'login' | 'dashboard' | 'manager' | 'editor' | 'daysPlanEditor' | 'daysPlan' | 'patient' | 'learning' | 'recall' | 'waiting' | 'metrics';
...
function authProblemMessage(message?: string) {
  ...
  return seed.app.copy.accountProblem;
}

function PhotoAnswer({ item, onPress, label }: { item: WhosWhoItem; onPress: (event: GestureResponderEvent) => void; label: string }) {
  return <Pressable accessibilityRole="button" accessibilityLabel={`Photo option: ${item.name}`} onPress={(event) => { touchFeedback(); onPress(event); }} style={({ pressed }) => [styles.photoAnswer, pressed && styles.pressed]}><Portrait uri={item.photoUri ?? undefined} name={item.name} size={132} /><Text style={styles.photoAnswerText}>{label}</Text></Pressable>;
}

function Header({ title, eyebrow, onBack, copy = seed.app.copy }: { title: string; eyebrow?: string; onBack?: () => void; copy?: any }) {
...
function hintMessage(item: WhosWhoItem, level: number, copy: any = seed.app.copy): string | null {
...
export default function SaathiWorkflow() {
  ...
  const copy = useMemo(() => translations[languageId] ?? translations.english, [languageId]);
  ...
  const loadDaysPlan = async (activePatientId: string) => {
    const storedItems = await listDaysPlanItems(activePatientId);
    if (storedItems.length) { setDaysPlanItems(storedItems); return; }
    const defaults = makeDaysPlanItems(seed.daysPlan.items, activePatientId);
    setDaysPlanItems(await saveDaysPlanItems(activePatientId, defaults));
  };

  useEffect(() => { void (async () => {
    await initializeItems();
    const stored = await getLocalSetting('patient-id'); const activePatientId = stored ?? seed.patient.id; if (stored) setPatientId(stored);
    await loadDaysPlan(activePatientId);
  ...
  const createCareCircle = async () => {
    ...
    setPatientId(data.patientId); await setLocalSetting('patient-id', data.patientId); await loadDaysPlan(data.patientId); await persistCareProfile(); return true;
  };
    ...
    try {
      if (authMode === 'signUp') {
        ...
        if (error || !data.user) setNotice(authProblemMessage(error?.message));
          ...
          if (links?.[0]?.patient_id) { setPatientId(links[0].patient_id); await setLocalSetting('patient-id', links[0].patient_id); await loadDaysPlan(links[0].patient_id); setScreen('dashboard'); }
  ...
  const beginDaysPlan = async (phase: 'morning' | 'evening') => { const started = await startDaysPlanSession(patientId, phase); setDaysPlanController(await readControllerState(patientId, 'days_plan')); setDaysPlanSession(started); await persistGameEvent(started.id, { type: 'prompt_shown', itemId: daysPlanItems[0]?.id ?? `${phase}-plan`, at: Date.now() }); };
  ...
  const goBack = useCallback(() => {
    ...
    const destinations: Partial<Record<Screen, Screen>> = { onboarding: 'language', login: 'language', dashboard: 'language', manager: 'dashboard', editor: 'manager', daysPlanEditor: 'dashboard', daysPlan: 'patient', patient: 'language', waiting: 'patient', metrics: 'dashboard' };
  ...
  const currentHint = activeItem ? hintMessage(activeItem, hintLevel, copy) : null;
  ...
  if (screen === 'metrics') return <Layout><Header copy={copy} title={copy.whosWhoMetrics} eyebrow={copy.dashboardTitle} onBack={goBack} /><CaregiverMetrics patientId={patientId} locale={language.dateFormatter} /></Layout>;
  if (screen === 'daysPlanEditor') return <Layout><Header copy={copy} title="Prepare today's plan" eyebrow={copy.dashboardTitle} onBack={goBack} /><DaysPlanEditor items={daysPlanItems} onSave={saveDaysPlan} onCancel={() => setScreen('dashboard')} /></Layout>;
  if (screen === 'daysPlan') return <Layout patient><Header copy={copy} title="Today's plan" eyebrow="Saathi" onBack={goBack} /><DaysPlanActivity items={daysPlanItems} patientName={patientDisplayName} controllerState={daysPlanController} onExit={() => setScreen('patient')} onPhaseStart={beginDaysPlan} onEvent={recordDaysPlanEvent} onComplete={finishDaysPlan} onAbandon={abandonDaysPlan} /></Layout>;
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { zIndex: 1 }]} keyboardShouldPersistTaps="handled">
        <View style={styles.languageHeader}>
          <View style={styles.appIcon}><Text style={styles.appIconText}>S</Text></View>
  ...
  if (screen === 'login') return (
    <Layout>
      ...
      <ActionButton
        label={loginRole === 'patient' ? copy.continue : (busy ? (authMode === 'signIn' ? copy.signingIn : copy.creatingAccount) : authMode === 'signIn' ? copy.signIn : copy.createAccount)}
        onPress={loginRole === 'patient' ? () => setScreen('patient') : authenticate}
        disabled={loginRole === 'caretaker' && busy}
      />
  ...
  if (screen === 'daysPlanEditor') return <Layout><DaysPlanEditor items={daysPlanItems} onSave={saveDaysPlan} onCancel={() => setScreen('dashboard')} /></Layout>;
  if (screen === 'daysPlan') return <Layout patient><DaysPlanActivity items={daysPlanItems} patientName={patientDisplayName} onExit={() => setScreen('patient')} onPhaseStart={beginDaysPlan} onEvent={recordDaysPlanEvent} onComplete={finishDaysPlan} onAbandon={abandonDaysPlan} /></Layout>;
  if (screen === 'dashboard') return <Layout><Header title={copy.dashboardTitle} eyebrow={`${setup.guardianName} · ${setup.relationship}`} /><Notice>Photos, voice notes, learning history, and review schedules are stored locally first.</Notice><View style={styles.panel}><Text style={styles.overline}>MEMORY LIBRARY</Text><Text style={styles.panelTitle}>{copy.whosWhoTitle}</Text><Text style={styles.body}>{`${items.length} active familiar memories are ready for gentle practice.`}</Text><ActionButton label={copy.manageWhosWho} onPress={() => setScreen('manager')} /></View><View style={styles.panel}><Text style={styles.overline}>TODAY</Text><Text style={styles.panelTitle}>Day’s Plan</Text><Text style={styles.body}>Prepare three familiar moments for the patient’s morning and evening.</Text><ActionButton label="Prepare today’s plan" onPress={() => setScreen('daysPlanEditor')} variant="secondary" /></View><ActionButton label={copy.patientMode} onPress={() => setScreen('patient')} variant="quiet" /><ActionButton label={copy.signOut} onPress={async () => { await supabase?.auth.signOut(); setScreen('language'); }} variant="quiet" /></Layout>;
...
const styles = StyleSheet.create({
  photoChoices: { flexDirection: 'row', flexWrap: 'wrap', gap: 14, justifyContent: 'center' }, hint: { gap: 10 },  backText: { color: theme.colors.leaf, fontSize: theme.type.guardian, fontWeight: '800' },
  ...
  bgTop: { position: 'absolute', top: 0, left: 0, width: '100%', height: 200, opacity: 0.15, resizeMode: 'cover' },
  bgBottom: { position: 'absolute', bottom: 0, left: 0, width: '100%', height: 200, opacity: 0.15, resizeMode: 'cover' },
