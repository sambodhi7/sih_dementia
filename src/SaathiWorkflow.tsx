...
type Screen = 'language' | 'onboarding' | 'login' | 'dashboard' | 'manager' | 'editor' | 'patient' | 'learning' | 'recall' | 'waiting' | 'skills-manager' | 'skills-home' | 'skill-invitation' | 'skill-active' | 'skill-complete';
...
export default function SaathiWorkflow() {
  ...
  useEffect(() => { void (async () => {
    ...
    await initializeSkillTransmission(localPatientId); await refresh(localPatientId); setReady(true);
  })(); }, []);
  ...
  const createCareCircle = async () => {
    ...
    setPatientId(data.patientId); await setLocalSetting('patient-id', data.patientId); await persistCareProfile(); await initializeSkillTransmission(data.patientId); await refresh(data.patientId); return true;
  };
    ...
    try {
      if (authMode === 'signUp') {
        ...
        if (error || !data.user) setNotice(authProblemMessage(error?.message));
          ...
          if (links?.[0]?.patient_id) { const linkedPatientId = links[0].patient_id; setPatientId(linkedPatientId); await setLocalSetting('patient-id', linkedPatientId); await initializeSkillTransmission(linkedPatientId); await refresh(linkedPatientId); setScreen('dashboard'); }
  ...
  const goBack = useCallback(() => {
    ...
    const destinations: Partial<Record<Screen, Screen>> = { onboarding: 'language', login: 'language', dashboard: 'language', manager: 'dashboard', editor: 'manager', patient: 'language', waiting: 'patient', 'skills-manager': 'dashboard', 'skills-home': 'patient' };
